...
export default function SecureMessaging() {
  ...
  useEffect(() => {
    ...
    s.on('message', async (data: any) => {
      // route only DM messages for selected peer (roomId = sorted pair)
      if (!peerId) return;
    ...
    s.on('key_shared', async (data: any) => {
      if (!peerId || data.peerId !== peerId) return;
      try {
        const k = await importKeyFromHex(data.keyHex);
        setKey(k);
      } catch {}
    });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error('[SecureMessaging] Failed to load users', e);
      }
    })();
  }, [me.id]);
  ...
  async function selectPeer(id: string) {
    ...
    try {
      ...
      console.error('[SecureMessaging] load history failed', e);
    }
  }
  ...
  async function send() {
    ...
    try {
      ...
      console.error('[SecureMessaging] send failed', e);
    }
  }
  ...
  return (
    <div className="h-full flex">
      ...
      <div className="w-72 border-r bg-gray-50 p-3 space-y-2">
        <div className="font-semibold">Secure Messaging (1:1)</div>
      ...
      <div className="flex-1 flex flex-col">
        <div className="h-14 border-b flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button disabled={!peerId || loadingKey} onClick={startQKD} className="px-3 py-1 text-sm rounded bg-purple-600 text-white disabled:opacity-50">{loadingKey?'Establishing...':'Start QKD (BB84)'}</button>
          </div>
        </div>
        <div className="flex-1 p-4 space-y-2 overflow-y-auto bg-white">
          {messages.map(m => (
            <motion.div key={m._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${m.sender===me.id?'justify-end':'justify-start'}`}>
        ...
        <div className="h-14 border-t flex items-center gap-2 px-3 bg-white">
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter') send(); }} placeholder={key?"Type a message":"Run QKD to enable messaging"} className="flex-1 border rounded px-3 py-2" disabled={!key} />
          <button onClick={send} className="px-4 py-2 rounded bg-purple-600 text-white disabled:opacity-50" disabled={!peerId || !key || !input.trim()}>Send</button>
        </div>
      </div>
    </div>
  );
